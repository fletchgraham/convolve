...
export default function ConvolutionGrid({
  originalGrid,
  renderedGrid,
  onCellChange,
  showBorders = true,
  ...
  const colCount = originalGrid[0]?.length || 0;

  // Calculate min and max values in the rendered grid for color mapping
  const flatValues = renderedGrid.flat().map((value) => parseFloat(value) || 0);
      ...
      {originalGrid.map((row, r) =>
        row.map((value, c) => (
          <Cell
            ...
            color={getColorForValue(renderedGrid[r]?.[c] || 0)}
          />
        ))
      )}
    </div>
